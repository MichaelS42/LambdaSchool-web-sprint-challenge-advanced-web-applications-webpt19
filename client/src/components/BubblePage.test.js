...
//     ]
// test("Fetches data and renders the bubbles", () => {
//   const {container, queryAllByTestId, rerender, act, screen} = render(<BubblePage />)
//   act(() => container = render(<BubblePage />))

//   await waitfor (() => {
//     const colors = container

// })

// act(() => container = render(<BubblePage ))
// await waitfor (() => {
//   const colorItems = contain
// })
jest.mock("./components/BubblePage");
const mockData = [];

test("renders bubblepage", () => {
  render(<BubblePage />)
})

test("renders ColorList and BUbbles", () => {
  render(<ColorList colors={[]}/>)
})

test("fetches data nad renders the bubbles", async () => {
mock
